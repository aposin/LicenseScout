...
If `showDocumentationUrl` is not configured the documentation URL is not included into the output.
...
A single license identifier "-" is interpreted as "no modify" (the detected licenses ae taken over unmodified). This can b used if only provider, notice or documentation URL should be configured.
